...
- **.NET SDK 10.0** (`dotnet --version` should report `10.0.x`). Apps target `net9.0-*` TFMs,
  which the .NET 10 SDK builds fine.
- **MAUI workloads**: `android`, `ios`, `maccatalyst`, `maui-windows`. Install/update with:
  ```
...
Caller app (Android):
```
dotnet build src/apps/Bingo.Caller.App/Bingo.Caller.App.csproj -f net9.0-android
```

Player app (Android):
```
dotnet build src/apps/Bingo.Player.App/Bingo.Player.App.csproj -f net9.0-android
```

To deploy to a connected device/emulator, use `dotnet build -t:Run -f net9.0-android` from the
