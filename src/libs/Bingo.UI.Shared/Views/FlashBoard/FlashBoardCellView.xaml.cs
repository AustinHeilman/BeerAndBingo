...
public partial class FlashBoardCellView : ContentView
{
	...
	private async Task AnimateInAsync(double scale, CancellationToken token)
	{
		...
		Color settledColor = Application.Current?.Resources?.TryGetValue("FlashBoardCellBGColor_Called", out object? color) == true && color is Color validColor
			? validColor
			: Colors.Transparent;
	...
	private async Task AnimateOutAsync()
	{
		...
		if (Application.Current?.Resources?.TryGetValue("FlashBoardCellBGColor_Uncalled", out object? fallback) == true &&
